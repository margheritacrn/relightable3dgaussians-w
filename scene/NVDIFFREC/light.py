...
class EnvironmentLight(torch.nn.Module):
    ...
    def shade_ref(self, gb_pos, gb_normal, diffuse, ks, kr, view_pos, incoming_light=None, specular=True):
        """
        Assuming a micorfacet reflectance model the function returns specular color + diffuse color.
        The diffuse color is provided as input, while the specular color is computed using split sum approximation method.
        Args:
            ...
            incoming_light: SH coefficients of environment light
        """
        # (H, W, N, C)
        wo = util.safe_normalize(view_pos - gb_pos)
        ...
        if specular:
            ...
            # Roughness adjusted specular env lookup, get specular light
            #miplevel = self.get_mip(roughness)
            #spec = dr.texture(self.specular[0][None, ...], reflvec.contiguous(), mip=list(m[None, ...] for m in self.specular[1:]), mip_level_bias=miplevel[..., 0], filter_mode='linear-mipmap-linear', boundary_mode='cube')
        ...
        rgb = specular_linear + (1.0 - specularity)*diffuse_linear
