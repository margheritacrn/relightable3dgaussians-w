...
def render_and_evaluate_test_scenes(cfg, eval_all=False):
    ...
    for view in tqdm(test_cameras):
        ...
        white_envlight_sh = np.zeros((25,3))
        white_envlight_sh[0, :] =  0.9 * math.sqrt(4 * math.pi)

        model.envlight.set_base(torch.tensor(white_envlight_sh, dtype=torch.float32, device="cuda"))
