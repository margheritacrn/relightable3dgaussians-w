...
def render_and_evaluate_test_scenes(cfg, eval_all=False):
    ...
    out_dir_name = "eval_gt_envmap_random_sun_angles"
    if eval_all:
        out_dir_name = "eval_gt_envmap_all_random_sun_angles"
    ...
    for view in tqdm(test_cameras):
        ...
        rand_sun_angle = random.choice(sun_angles)

        print(f"Sun angle {rand_sun_angle}")
        ...
        gt_envmap_sh_rot = spaudiopy.sph.rotate_sh(gt_envmap_sh_rot, rand_sun_angle[2], rand_sun_angle[0], rand_sun_angle[1], 'real')

        # Save best_envmap:
        np.save(os.path.join(renders_path, "best_envmap" + view.image_name+".npy"), gt_envmap_sh_rot)

        render_rand_sun_angle_envmap = sh_utility.sh_render(gt_envmap_sh_rot.T, width = 360)
        render_rand_sun_angle_envmap = torch.tensor(render_rand_sun_angle_envmap**(1/ 2.2))
        render_rand_sun_angle_envmap =  np.array(render_rand_sun_angle_envmap*255).clip(0,255).astype(np.uint8)
        render_rand_sun_angle_envmap = Image.fromarray(render_rand_sun_angle_envmap)
        render_rand_sun_angle_envmap.save(os.path.join(renders_path, "rand_sun_angle_rot_envmap"+view.image_name+".jpg"))
        ...
        used_angles.append(rand_sun_angle)
